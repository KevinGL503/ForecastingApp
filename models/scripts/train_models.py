...
def improve_model(X_train, y_train, model):
    ...
    #Check if the model has these parameters in it
    if 'n_estimators' not in model.get_params().keys() or 'max_depth' not in model.get_params().keys() or 'min_samples_split' not in model.get_params().keys():
        return model
    else:
